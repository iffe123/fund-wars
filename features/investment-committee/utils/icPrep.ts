...
export const buildICPrepBrief = (deal: PortfolioCompany): ICPrepBrief => {
  ...
  const leverage = deal.ebitda > 0 ? deal.debt / deal.ebitda : 0;
  const equityCheck = Math.max(deal.currentValuation - deal.debt, 0);
  ...
  const thesis = `Back ${deal.name} at ${entryMultiple.toFixed(1)}x with a ${formatMoneyShort(equityCheck)} equity check, then create value through operational discipline instead of hoping for multiple expansion.`;
